...
const seed = (path: string, data: Record<string, unknown>) =>
  testEnv.withSecurityRulesDisabled((ctx) =>
    setDoc(doc(ctx.firestore(longPolling), path), data),
  );
...
describe("firestore.rules（emulator・層②）所有者ベースの書き込み制御", () => {
  ...
  it("read は未認証でも許可される", async () => {
    const db = testEnv.unauthenticatedContext().firestore(longPolling);
  ...
  it("create は userId == auth.uid なら許可される", async () => {
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
  ...
  it("create で userId が空文字列だと拒否される", async () => {
    // auth.uid は空文字列になれないが、userId フィールドに "" を渡すと
    // ownerIsSelf()（uid != ""）で弾かれ、isValidMatch()（size > 0）でも弾かれる。
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
    await assertFails(setDoc(doc(db, path), validMatch("")));
  });

  it.each([
    { label: "2 要素（不足）", first: ["DARKNESS", "FIRE"] },
    { label: "4 要素（過多）", first: ["DARKNESS", "FIRE", "HATE", "WATER"] },
  ])("create で first が $label だと拒否される", async ({ first }) => {
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
    await assertFails(setDoc(doc(db, path), { ...validMatch("u1"), first }));
  });

  it("create で second の要素数が 3 でないと拒否される", async () => {
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
    await assertFails(
      setDoc(doc(db, path), {
        ...validMatch("u1"),
        second: ["PSYCHIC", "GRAVITY"],
      }),
    );
  });

  it("create で ratio が bool でないと拒否される", async () => {
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
    await assertFails(
      setDoc(doc(db, path), { ...validMatch("u1"), ratio: "true" }),
    );
  });
  ...
  it("update で winner を不正値に変えると拒否される", async () => {
    await seed(path, validMatch("u1"));
    const db = testEnv.authenticatedContext("u1").firestore(longPolling);
    await assertFails(updateDoc(doc(db, path), { winner: "DRAW" }));
  });

  it("他人は所有者の doc を更新できない", async () => {
    await seed(path, validMatch("u1"));
    const db = testEnv.authenticatedContext("u2").firestore(longPolling);
  ...
  it("userId を持たない過去データは閲覧できるが編集・削除できない", async () => {
    ...
    const reader = testEnv.unauthenticatedContext().firestore(longPolling);
    await assertSucceeds(getDoc(doc(reader, path)));

    const writer = testEnv.authenticatedContext("u1").firestore(longPolling);
